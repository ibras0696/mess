...
- Сделано:
  ...
  - Исправил e-mail уведомления: шлём только оффлайн участникам, импортировал AttachmentRepository, списки вложений перевёл на default_factory.
  - Обновил порты Mailhog (SMTP 1826, UI 8126) в .env/.env.example, чтобы не конфликтовали с хостом.
- В работе:
  - Доп. hardening (headers/rate limits) — опционально.
- Блокеры/риски:
  - Mailhog amd64 на arm64 предупреждение остаётся (не критично).
- Следующие шаги:
  - При необходимости расширить rate limits/headers и включить online_status.
- Артефакты:
  - PR: -
  - API_CONTRACT: обновлён? нет
  - openapi.json: обновлён? нет
  - Alembic миграции: нет
  - docker-compose/Makefile: порты Mailhog (env)
